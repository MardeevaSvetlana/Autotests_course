...
def scrabble(word):
    """ Формируем словарь (ключи-количество очков, значения-строки из букв, за которые начисляются очки).
         Затем получаем пары ключ-значения словаря, значения которых содержат буквы слова и суммируем ключи."""
    my_dict = {1: 'авеёинорст',
               2: 'дклмпу',
               3: 'бгья',
               4: 'йы',
               5: 'жзхцч',
    ...
    points = (sum([k for item in word for k, v in my_dict.items() if item in v]))
